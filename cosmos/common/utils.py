...
synchronized = lockutils.synchronized_with_prefix('cosmos-')
...
def check_metadata_properties(metadata=None):
    """Checks that the metadata properties are valid."""
...
def get_root_helper():
    return 'sudo cosmos-rootwrap %s' % CONF.rootwrap_config
...
def log_unsupported_driver_warning(driver):
    ...
    if not driver.supported:
        ...
        LOG.warning("Volume driver (%(driver_name)s %(version)s) is "
                    ...
                    "next release of Cosmos.  Use at your own risk.",
                    {'driver_name': driver.__class__.__name__,
